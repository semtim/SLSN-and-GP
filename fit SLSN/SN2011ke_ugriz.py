...
X = np.linspace(min(x[:,1]), max(x[:,1]),266).reshape(-1,1)   #step=1day
...
steps = 42
s=3
T = np.linspace(min(x[:,1]), max(x[:,1]),266)
f = open("SN2011ke_r.dat", 'w')
...
f = open("SN2011ke_g.dat", 'w')
for t in range(steps):
      f.write(str(T[t*s]))
      f.write(' ')
      f.write( str( -2.5*np.log10(data_pred.odict["g"].y)[t*s] ) )
      f.write(' ')
      f.write('0')
...
f = open("SN2011ke_z.dat", 'w')
for t in range(steps):
      f.write(str(T[t*s]))
      f.write(' ')
      f.write( str( -2.5*np.log10(data_pred.odict["z"].y)[t*s] ) )
      f.write(' ')
      f.write('0')
